  ...
  <div class="relative">
    <header class="pt-5 px-5">
      <nav class="flex flex-wrap justify-between items-center gap-4">
        ...
        <button class="group z-10" @click="toggleTheme()">
          <svg
            v-if="!isDark"
          ...
          >
            <path
              class="fill-gray-900 group-hover:fill-gray-700 transition-all"
          ...
          <svg v-else width="19" height="19" xmlns="http://www.w3.org/2000/svg">
            <path
              class="fill-gray-100 group-hover:fill-gray-300 transition-all"
    ...
    <main class="max-w-screen-xl px-4 md:px-8 mx-auto pt-20 py-14">
      <div class="grid lg:grid-cols-2 gap-16 md:gap-8">
        <div class="text-center lg:text-left z-10">
          <h1 class="text-4xl md:text-6xl font-bold mb-6">
            <span class="text-clrPurple">Kanban</span> <br />
            Task Management System
          </h1>
          <p class="text-gray-400 text-lg md:text-2xl">
            A tool to manage all your project tasks in a simpliest way possible
          </p>
        </div>
        <RegisterForm v-if="showRegisterForm" class="z-10">
          <p class="text-end text-xs z-10 relative">
            Already have an account?
            <span
              class="text-clrPurple cursor-pointer hover:underline underline-offset-4"
              @click="showRegisterForm = !showRegisterForm"
              >Sign in now</span
            >
          </p>
        </RegisterForm>
        <LoginForm v-else class="z-10">
          <p class="text-end text-xs z-10 relative">
      ...
      <div
        v-if="isDesktop"
        class="absolute w-48 h-48 bg-clrPurple/30 rounded-full blur-3xl -translate-x-1/2 -translate-y-1/2 z-0"
        :style="{
          left: `${x}px`,
          top: `${y}px`,
        }"
      ...
      <div
        v-else
        class="absolute top-0 left-[50%] -translate-x-1/2 w-44 h-44 animate-pulse bg-clrPurple/40 rounded-full blur-3xl"
      ></div>
    </main>
  </div>
</template>
