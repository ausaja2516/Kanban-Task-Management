<template>
  <div v-if="pending" class="grid place-content-center min-h-screen">
    <span
      class="w-10 h-10 border-x border-clrPurple rounded-full animate-spin"
    ></span>
  </div>
  <div v-if="error" class="grid place-content-center min-h-screen text-center">
    <span class="text-3xl mb-4">🤔</span>
    <span class="text-xl"
      >Oops! an error occured <br />
      Try checking your internet connection</span
    >
  </div>
  <div v-if="user" class="grid grid-cols-[250px,1fr]">
    <aside class="min-h-screen bg-gray-50 dark:bg-gray-800 py-4">
      <div class="ps-5">
        <svg
          v-if="!isDark"
          class="z-10"
        ...
        <!-- profile  -->
        <div></div>
        <button type="button" @click="toggleTheme()" class="py-px px-2 mt-4">
          changetheme
        </button>
      </div>
      <p>This is the dashboard of: "{{ user?.displayName }}"</p>
      <button
        @click="Users.logout"
        type="button"
        class="px-4 py-2 text-white bg-clrPurple rounded-md"
      >
        Sign Out
      </button>
    </aside>
...
<script setup>
import WelcomeMsg from "@/components/WorkspaceModal.vue";
...
const {
  ...
  pending,
  error,
} = useDocument(doc(collection(db, "Users"), route.meta.user.uid));
console.log(user);
</script>
